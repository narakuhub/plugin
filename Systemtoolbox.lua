		...
		-------------------------------------------------------------------------
		-- PEMANGGILAN OBJEK UI (100% SINKRON DENGAN STRUKTUR SCREENGUI TERBARU ANDA)
		-------------------------------------------------------------------------
		...
		local SearchButton = LMG2L["GetButton_c"] -- TextButton CARI di dalam SearchAsset_b
		...
		local TemplateFrame = LMG2L["Frame_11"] -- Target template item list abang
		...
		local CurrentSessionId = 0 -- Token pengunci thread pencarian agar anti-nyasar antar tab menu
		local SavedAssets = {
...
local function RenderAssets(searchQuery)
    ...
    -- Generate ID sesi pencarian unik untuk thread saat ini
    ...
    local query = ""
    if searchQuery and searchQuery ~= "cari asset.." then
        query = searchQuery:lower():match("^%s*(.-)%s*$") or ""
    end

    local function UpdateCanvas()
        -- Pastikan UI masih berada di kategori yang sama sebelum menghitung Canvas
    ...
    for _, assetId in ipairs(targetList) do
        task.spawn(function()
            -- PERBAIKAN: Gunakan Caching agar tidak memanggil MarketplaceService berkali-kali
            ...
            -- PROTEKSI BUG NYASAR: Batalkan render jika user sudah menekan/berpindah ke tab menu lain!
            if thisSession ~= CurrentSessionId or CurrentCategory ~= targetCategoryAtCall then 
                return 
            end
            
            if success and info then
                -- VALIDASI KATA KUNCI FIX
                if query ~= "" then
                ...
                -- Clone manual berdasarkan struktur instansiasi LMG2L Anda
                ...
                -- Menyalin anak objek (tetap pakai struktur LMG2L Anda)
                ...
                -- PERBAIKAN: Hapus UpdateCanvas dari sini agar rendering tidak lag
            end
        end)
    end
    
    -- PERBAIKAN: Panggil UpdateCanvas sekali di akhir proses saja
...
local function SwitchTab(tabName)
    ...
    RenderAssets()
		...
		-- Deteksi ketikan dinamis pada Search Box (Pencarian instan otomatis kata kunci nama asset)
